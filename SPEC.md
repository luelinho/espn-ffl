...
**Activated 2026-09-17, with explicit confirmation** (public repo, secrets, and all — see CLAUDE.md's "Public GitHub Pages hosting" entry for the full story including two real bugs found deploying it). `.github/workflows/daily.yml` runs the full pipeline once a day (cron 06:05 UTC) and commits the outputs; `.github/workflows/pages-deploy.yml` deploys to GitHub Pages on every push to `main` that touches `dashboard.html`, from either that daily job or `live_refresh.py`'s own live-cadence pushes. Live at `https://luelinho.github.io/espn-ffl/`.
*Exit: three consecutive clean automated runs.* ✅ both locally and on GitHub Actions.
...
**Update 2026-09-14 — visual redesign + opt-in live-refresh mode.** Owner supplied a reference image (sidebar app-shell, gradient KPI cards, ring/bar/dot-matrix charts); rebuilt `build_dashboard.py`'s CSS/JS to match that visual language while keeping every existing route/component/data-binding intact. Every chart is driven by real digest numbers only — no fabricated week-over-week deltas, no placeholder history beyond weeks that actually exist. Also added `src/live_refresh.py`: an opt-in loop (default every 240s, owner-requested 3-5 minute range, 2026-09-14) that re-runs the full pipeline for game-day viewing, paired with a matching `<meta http-equiv="refresh">` on the page itself and `localStorage`-based tab/league persistence so the auto-reload doesn't reset your view. This is separate from, and doesn't change, the Phase 4 scheduled daily job.

Two real regressions found and fixed while verifying this against the "nothing broken" bar: (1) the sidebar was pure `display:none` below 820px with no replacement, a genuine dead end for reaching any tab but Home at that width — fixed with a horizontal-scrolling fallback nav, not just restored visibility; (2) the sidebar/topbar weren't pinned, so navigating away from a scrolled position required scrolling back to the top first — fixed with an app-shell scroll layout (fixed sidebar/topbar, independently scrolling content). Verified in-browser across all 5 pages, both leagues, the Managers dropdown, and the mobile-width fallback nav.

**Update 2026-09-14 — Home redefined as both-leagues matchups only.** Owner request: "see all my players and opponent for the week at once" for both leagues, then "the only thing that should be on the home page is both leagues' matchups." Home now ignores the league switcher entirely (the only page that does) and always renders both leagues: a position-aligned head-to-head card per league (`matchupComparisonCard()` — your starters vs. your opponent's, row-by-row by lineup slot: QB/RB/RB/WR/WR/TE/D-ST/K/FLEX, using the Phase-1-confirmed slot-ID labels, not a guessed mapping) side by side, then both leagues' full weekly scoreboards side by side below. Confirmed with the owner before building (layout: side-by-side columns for both sections; disposition of the removed KPI/chart/alerts content: deleted from Home entirely, not relocated — it already lives on My Team/Managers/Analytics). Removed now-dead JS (`weeklyBarHistory`, `dotMatrix`) and CSS rather than leaving unused code in place.
